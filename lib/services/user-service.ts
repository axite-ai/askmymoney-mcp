...
/**
 * Map a database row to a PlaidItem with decrypted access token
 */
function toPlaidItem(row: typeof plaidItems.$inferSelect): PlaidItem {
  return {
    ...row,
    accessToken: EncryptionService.decrypt(row.accessToken),
    newAccountsAvailable: row.newAccountsAvailable as PlaidItem['newAccountsAvailable'],
  };
}
