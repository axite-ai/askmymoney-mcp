...
import { createAuthNonce } from "@/lib/utils/auth-nonce";
import {
...
import { FREE_PLAN_NAME, PLAN_LIMITS } from "@/lib/utils/plan-limits";
...
const handler = withMcpAuth(auth, async (req, session) => {
  ...
  return createMcpHandler(async (server) => {
    ...
    async ({ timeframe = "30d", includeProjections = false }: {
      ...
      try {
        ...
        logger.error("[Tool] askmymoney_get_account_balances error", { error });
  ...
  server.registerTool(
    "askmymoney_get_transactions",
    {
      ...
      inputSchema: {
        startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional().describe("Start date in YYYY-MM-DD format. Defaults to 30 days ago."),
        endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional().describe("End date in YYYY-MM-DD format. Defaults to today."),
        limit: z.number().int().min(1).max(500).optional().describe("Maximum number of transactions to return (1-500). Defaults to 100."),
    ...
    async ({ startDate, endDate, limit, offset, category, paymentChannel, includePending = true }: {
      startDate?: string;
      endDate?: string;
      limit?: number;
      offset?: number;
      ...
      try {
        ...
        if (Number.isNaN(parsedStart.getTime()) || Number.isNaN(parsedEnd.getTime())) {
          return createErrorResponse("Invalid date format. Use YYYY-MM-DD for startDate and endDate.");
        }
        if (parsedStart > parsedEnd) {
          return createErrorResponse("startDate must be on or before endDate.");
        }
        ...
        if (userItems.length === 0) {
          return createErrorResponse("No connected accounts. Use askmymoney_connect_item to link a bank account first.");
        }
        ...
        if (userAccountIds.length === 0) {
          return createErrorResponse("No connected accounts found. Try reconnecting via askmymoney_connect_item.");
        }

        const whereClauses = [
          inArray(plaidTransactions.accountId, userAccountIds),
          gte(plaidTransactions.date, parsedStart),
          lte(plaidTransactions.date, parsedEnd),
        ];

        if (!includePending) {
          whereClauses.push(drizzleEq(plaidTransactions.pending, false));
        }
        if (category) {
          whereClauses.push(drizzleEq(plaidTransactions.categoryPrimary, category));
        }
        if (paymentChannel) {
          whereClauses.push(drizzleEq(plaidTransactions.paymentChannel, paymentChannel));
        }
        ...
        const totalMatching = Number(totalMatchingResult[0]?.count ?? 0);

        const fetchedTransactions = await db.query.plaidTransactions.findMany({
          where: whereCondition,
        ...
        const hasMore = fetchedTransactions.length > effectiveLimit;
        const limitedTransactions = fetchedTransactions.slice(0, effectiveLimit);
        ...
        const structuredContentForModel = {
          ...
          pagination: {
            ...
            offset: offsetValue,
            hasMore,
            nextOffset: hasMore ? offsetValue + effectiveLimit : null,
          },
          metadata: {
        ...
        logger.error("[Tool] askmymoney_get_transactions error", { error });
        return createErrorResponse(
  ...
  server.registerTool(
    "askmymoney_get_spending_insights",
    {
    ...
    async ({
      ...
      try {
        ...
        logger.error("[Tool] askmymoney_get_spending_insights error", { error });
        return createErrorResponse(
  ...
  server.registerTool(
    ...
    async () => {
      try {
        ...
        const totalBalance = allAccounts.reduce((sum, account) => sum + (account.balance || 0), 0);
        ...
        logger.error("[Tool] askmymoney_check_account_health error", { error });
        return createErrorResponse(
  ...
  server.registerTool(
    "askmymoney_get_investment_holdings",
    {
    ...
    async ({
      ...
      try {
        ...
        logger.error("[Tool] askmymoney_get_investment_holdings error", { error });
        return createErrorResponse(
  ...
  server.registerTool(
    ...
    async () => {
      try {
        ...
        logger.error("[Tool] askmymoney_get_liabilities error", { error });
        return createErrorResponse(
    ...
    server.registerTool(
      ...
      async () => {
        try {
          // Check authentication (subscription check skipped when subscriptions disabled)
          ...
          // When subscriptions are disabled, return free plan info
          if (!FEATURES.SUBSCRIPTIONS) {
            return createSuccessResponse(
              `You're on the Free plan with up to ${PLAN_LIMITS.free.maxAccounts} connected accounts. No subscription required.`,
              {
                currentPlan: "Free",
                message: `You're on the Free plan. You can connect up to ${PLAN_LIMITS.free.maxAccounts} financial accounts at no cost.`,
              }
            );
          }
          ...
          logger.error("[Tool] askmymoney_manage_subscription error", { error });
          return createErrorResponse(
      ...
      async () => {
        try {
          ...
          if (data.items.length === 0) {
            ...
            if (isFree) {
              message = `You've reached the maximum of ${data.planLimits.max} free accounts. Remove an existing connection to add a new one.`;
            } else if (data.deletionStatus.canDelete) {
          ...
          const metaForWidget = {
            items: data.items,
            authNonce: createAuthNonce(session.userId),
            baseUrl: baseURL,
          };

          return createSuccessResponse(message, { ...structuredContentForModel, items: data.items }, metaForWidget);
        } catch (error) {
          logger.error("[Tool] askmymoney_connect_item error", { error });
          return createErrorResponse(
    ...
    // ============================================================================
    // TEST WIDGET (development only)
    // ============================================================================
    if (process.env.NODE_ENV === "development") {
      server.registerTool(
        "askmymoney_test_widget",
        {
          title: "Test Widget",
      ...
      // ============================================================================
      server.registerTool(
        "askmymoney_advanced_test_widget",
        {
  ...
  server.registerTool(
    "askmymoney_track_recurring_payments",
    {
    ...
    async ({
      ...
      try {
        ...
        logger.error("[Tool] askmymoney_track_recurring_payments error", { error });
        return createErrorResponse(
  ...
  server.registerTool(
    ...
    async ({
      ...
      try {
        ...
        logger.error("[Tool] askmymoney_business_cash_flow error", { error });
        return createErrorResponse(
  ...
  server.registerTool(
    "askmymoney_categorize_expenses",
    {
    ...
    async ({
      ...
      try {
        ...
        logger.error("[Tool] askmymoney_categorize_expenses error", { error });
        return createErrorResponse(
  ...
  server.registerTool(
    "askmymoney_evaluate_payment_risk",
    {
      ...
      description: "Real-time ML risk scoring before initiating ACH payments. Predicts likelihood of returns/bounces using network intelligence from billions of transactions. Reduces failed payments by 50%. Requires authentication and connected bank accounts. Note: accountId must be a valid Plaid account_id from user's connected accounts (use askmymoney_get_account_balances to see available account IDs).",
      inputSchema: {
        accountId: z.string().describe("Plaid account_id from user's connected accounts (e.g., 'BxBXxLj1m68yDjeybBQC6eucT6K5tQhVCEBBv'). Use askmymoney_get_account_balances to see available accounts."),
    ...
    async ({
      ...
      try {
        ...
        if (accessTokens.length === 0) {
          return createErrorResponse("No Plaid accounts connected. Connect your bank account first using the askmymoney_connect_item tool.");
        }
        ...
        logger.error("[Tool] askmymoney_evaluate_payment_risk error", { error });
        return createErrorResponse(
...
// Export type for frontend type inference (used by mcp-ui-hooks)
