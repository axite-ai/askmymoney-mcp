...
async function handleItemAddResult(
  ...
  try {
    ...
    // Check if this is update mode (item already exists for this user)
    const existingItems = await UserService.getUserPlaidItems(session.userId, false);
    ...
    if (existingItem) {
      ...
      console.log('[Link Webhook] Update mode success - cleared error state for item:', itemId);
    } else {
      // NEW CONNECTION MODE: Save new item (status set to 'active' immediately)
