...
export default function Transactions() {
  ...
  const toolOutput = toolInfo.isSuccess
    ? (toolInfo.output as ToolOutputData | undefined)
    : undefined;
        ...
        {isFullscreen && metadata && (
          <div className="mb-6 space-y-3">
            <Input
              ...
              onChange={(e) => setUiState(s => s ? { ...s, searchQuery: e.target.value } : null)}
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
              <Select
                ...
                onChange={(val: any) => setUiState(s => s ? {...s, selectedCategory: val?.value ?? null} : null)}
                options={[
              ...
              <Button
                ...
                onClick={() => setUiState(s => s ? {...s, showPendingOnly: !s.showPendingOnly} : null)}
                size="sm"
              ...
              <div key={date}>
                    ...
                    {(isFullscreen || expandedDateGroupsSet.has(date) ? txs : txs.slice(0, MAX_VISIBLE_INLINE)).map((tx, index) => {
                      ...
                      return (
                        <div
                          ...
                          onClick={() => setUiState(s => s ? {...s, expandedTx: isExpanded ? null : tx.transaction_id} : null)}
                        >
                ...
                {!isFullscreen && !expandedDateGroupsSet.has(date) && txs.length > MAX_VISIBLE_INLINE && (
                  <Button
                    ...
                    onClick={() => {
                      setUiState(s => s ? { ...s, expandedDateGroups: [...s.expandedDateGroups, date]} : null);
                    }}
                  >
