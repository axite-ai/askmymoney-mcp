...
export default function Home() {
  ...
  const maxHeight = (useOpenAiGlobal("maxHeight") as number | string | undefined) ?? undefined;
  const [displayMode, requestDisplayMode] = useDisplayMode();

  // Check if we're in ChatGPT context
  const isChatGptApp = typeof window !== "undefined" && !!(window as any).openai;

  // Extract name from tool output
  // Note: toolInfo.output IS the structuredContent directly in Skybridge
  const toolOutput = toolInfo.isSuccess
    ? (toolInfo.output as { name?: string } | undefined)
    : undefined;
