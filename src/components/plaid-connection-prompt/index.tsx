...
import { useToolInfo, useOpenExternal } from "@/src/mcp-ui-hooks";
import {
  Business,
  Check,
  ShieldCheck,
...
interface PlaidConnectionPromptOutput {
  connectUrl?: string;
  baseUrl?: string;
}
...
export default function PlaidConnectionPrompt() {
  ...
  const toolOutput = toolInfo.isSuccess
    ? (toolInfo.output as PlaidConnectionPromptOutput | undefined)
    : undefined;
  ...
  const handleConnect = () => {
    ...
    // Open the authenticated link using Skybridge openExternal
